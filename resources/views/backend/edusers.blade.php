...
@endsection
@section('content')
<p class="text-2xl  sm:text-2xl  md:text-3xl lg:text-3xl font-body mb-5"> แก้ไข Users  </p>
<div class=" p-3 flex justify-center">
    <div class="max-w-md p-6 bg-white border border-gray-200 rounded-lg shadow ">
        <form action="{{route('user.updateusers',$users->id)}}" method="POST" enctype="multipart/form-data">
            @csrf
            <div class="flex items-center">
                <img class="w-20 rounded-full" src="{{ $users->avatar ?? asset('/avatars/default.gif') }}" alt="Profile Picture" id="previewImage">
                <div class="p-5">
            ...
            <input type="text" value="{{$users->username}}" id="username" name="username" class="m-2 bg-gray-50 border border-gray-300  focus:outline-none focus:ring focus:ring-violet-300 text-gray-900 text-base sm:text-base md:text-lg lg:text-lg rounded-md  block w-full p-2  ">
            @error('username')
            ...
            <input type="email" id="email" value="{{$users->email}}" name="email" class="m-2 bg-gray-50 border border-gray-300  focus:outline-none focus:ring focus:ring-violet-300 text-gray-900 text-base sm:text-base md:text-lg lg:text-lg rounded-md  block w-full p-2  ">
            @error('email')
            <div class="font-body text-red-700 text-base sm:text-base md:text-base lg:text-base">
                <span>{{$message}}</span>
            </div>
